...
These live in ``core`` (not ``cli``) so components can use them in
``expose_cli()`` without importing upward from the CLI layer.
"""
...
class FaultsFileType(click.ParamType):
    name = "faults_file"
    ...
    def convert(self, value: str, param: click.Parameter | None, ctx: click.Context | None) -> str:
        path = Path(value).expanduser()
        if not path.exists():
            self.fail(f"Faults file does not exist: {value}", param, ctx)
        if not path.is_file():
            self.fail(f"Path is not a file: {value}", param, ctx)
        if path.suffix.lower() not in (".yaml", ".yml"):
            self.fail(f"Faults file must be a YAML file (.yaml or .yml): {value}", param, ctx)
        return str(path)
